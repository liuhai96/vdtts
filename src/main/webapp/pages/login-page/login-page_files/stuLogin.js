...
var loginurl = $("#path").val()+"/userLogin";
...
function submit($,params){
    ...
    $.get(loginurl,params , function (json) {
        if (json.errorcode==0) {
        	...
        	localStorage.setItem('stuIdcard',idcard);
        	localStorage.setItem('stuPhone',phone);
            }
            window.location.href = path+"/studentCoaInfo";
        }else{
            layer.msg(json.message, {
                icon: 2,
                time: 1000 //2秒关闭（如果不配置，默认是3秒）
                ...
                alert(json.msg);//提示信息
                skipPage($("#path").val()+"/"+json.data.url);//路径跳转路径
            });
        }
    }, 'json');
}
